...
BOOL PopFindReplaceAllText(HWND hwndEdit, int iSearchOffset, LPFINDREPLACE pfr)// ReplaceAll by Tetr@pod すべて置換が遅すぎるため
	...
	ReplaceWith = (LPTSTR )malloc(lstrlen(pfr->lpstrReplaceWith) + 1);
	if (ReplaceWith == NULL) {
		return FALSE;
	}
    lstrcpy(ReplaceWith, pfr->lpstrReplaceWith);
	...
	dest.ptr = (LPTSTR)malloc(dest.length + 1);
	if(dest.ptr == NULL) return FALSE;
	...
	Footy2GetText(activeFootyID, dest.ptr, LM_CRLF, dest.length + 1);
		...
		// Search the document for the find string
	pattern.ptr = (LPTSTR)malloc(lstrlen(pfr->lpstrFindWhat) + 1);
	if(pattern.ptr == NULL){
		free(dest.ptr);
		return FALSE;
	}
	...
	//	iSearchOffset = Footy2GetCaretThrough(activeFootyID) - 1;
		...
		LPTSTR ratemp;// 置換後の文字を保持する一時的な変数
		ratemp = (LPTSTR)malloc(dest.length + 1);// ratemp作成
		...
		// ratempに置換後の文字を作成
		_tcsncpy(ratemp, dest.ptr, frReturn.offset);// 前の部分
		_tcscat(ratemp, ReplaceWith);// 置換部分
		_tcscat(ratemp, dest.ptr + frReturn.offset + pattern.length);// 後ろの部分
		ratemp[dest.length - 1] = dest.ptr[dest.length - 1];

		free(dest.ptr);// dest.ptr解放
		dest.ptr = (TCHAR *)malloc(dest.length + 1);// dest.ptr作成
		if (dest.ptr == NULL) {// dest.ptr確保失敗
			free(ratemp);// ratemp解放
			break;
		}
		// memset(dest.ptr, 0, dest.length + 1);// 一応dest.ptrを0フィル
		_tcscpy(dest.ptr, ratemp);// ratempをdest.ptrにコピー
